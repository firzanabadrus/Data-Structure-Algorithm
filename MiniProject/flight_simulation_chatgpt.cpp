...
void sortQueueByPriority() {
        for (int i = front; i < rear; i++) {
            for (int j = i + 1; j <= rear; j++) {
                // Sort based on fuel and passengers (priority formula)
                if (items[i]->fuel > items[j]->fuel || 
                    (items[i]->fuel == items[j]->fuel && items[i]->passenger < items[j]->passenger)) {
...
void Queue::enQueue(Flight* f) {
    if (!isFull()) {
        rear++;
        items[rear] = f;
        sortQueueByPriority(); // Re-sort the queue after adding a new flight
    } else {
        cout << "Queue is full...\n\n";
    }
}
