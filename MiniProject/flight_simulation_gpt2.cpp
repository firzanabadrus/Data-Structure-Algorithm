...
class Queue {
    ...
    public:
        ...
        bool isFull(void);
        bool isEmpty(void);
        void prioritizeQueue(void); // New method to reorder queue based on fuel levels.
};
...
void Queue::enQueue(Flight* f) {
    if (!isFull()) {
        rear++;
        items[rear] = f;
        prioritizeQueue(); // Reorder queue after adding a new flight.
    } else {
        cout << "Queue is full...\n\n";
    }
}
...
void Queue::prioritizeQueue(void) {
    // Bubble sort to reorder based on fuel levels (ascending order).
    for (int i = front; i < rear; i++) {
        for (int j = front; j < rear - (i - front); j++) {
            if (items[j]->fuel > items[j + 1]->fuel) {
                // Swap flights based on fuel levels.
                Flight* temp = items[j];
                items[j] = items[j + 1];
                items[j + 1] = temp;
            }
